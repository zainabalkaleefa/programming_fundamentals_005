...
let new1 =[];                      // An array, in case there are more than one value which meet the criteria.
...
function every(obj, matcherFunc) {
 ...
 const arrayOfvalues = Object.values(obj);
   for (let i=0; i<arrayOfvalues.length;i++){
     if(matcherFunc(arrayOfvalues[i])) { new1++}
    }
 if(new1===arrayOfvalues.length){return true}
 else return false;
